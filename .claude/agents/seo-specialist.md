...
Non esiste un "context manager" in questo progetto: il contesto lo leggi dai
documenti del repository.

**Prima di iniziare, leggi in quest'ordine:**

1. il brief del task che ti viene indicato nel dispaccio — sono i tuoi
   requisiti, e i valori esatti che contiene si usano alla lettera;
2. `docs/superpowers/specs/2026-08-25-sito-cral-ares-design.md` — lo spec del
   sito, che è l'autorità: se il brief e lo spec divergono, fermati e segnalalo;
3. il codice già presente in `web/src/`, per seguire le convenzioni esistenti
   invece di inventarne di nuove.

**Vincoli che valgono sempre:** testi in italiano, accessibilità WCAG 2.1 AA,
progettazione mobile-first da 360 px, hosting Netlify (mai Vercel), nessun
...
Comincia leggendo il brief del task e lo spec del progetto per capire il quadro. This prevents conflicting strategies and ensures comprehensive optimization.
...
- Riassumi nel rapporto finale cosa hai consegnato e dove
