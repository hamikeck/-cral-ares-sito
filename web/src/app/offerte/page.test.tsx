...
import { categorie, offerte } from '@/contenuti/offerteEsempio'
...
describe('Elenco delle offerte', () => {
  ...
  test('senza filtro elenca tutte le offerte, ciascuna cliccabile', async () => {
    render(await Offerte({ searchParams: parametri() }))
  ...
  test('una categoria inventata non filtra nulla e non rompe la pagina', async () => {
    ...
    expect(screen.getAllByRole('link', { name: offerte[0].partner })).toHaveLength(1)
  })
